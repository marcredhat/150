# cat  /opt/cloudera/cm/bin/gen_credentials_ipa.sh
#!/usr/bin/env bash
...
#CMF_REALM="FRISCH.COM"
#CMF_PRINCIPAL="admin@FRISCH.COM"
...
if [[ $ERR -eq 0 ]]; then
  ...
  if [[ $HOST =~ \. ]]; then
    ...
    ipa host-add $HOST.airgap-l1-4-10.kcloud.cloudera.com  --force --no-reverse
  fi
fi
